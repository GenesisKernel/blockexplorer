...
@app.route("/genesis/database/<int:id>/blocks")
def blocks(id):
...
@app.route('/dt/genesis/database/<int:id>/blocks')
def dt_blocks(id):
    model = BlockChain
    column_ids = ['id', 'time', 'hash', 'node_position', 'ecosystem_id',
                  'data', 'tx']
    ...
    rowTable.blocks_post_query_process(hash_ids=2,
                                       time_ids=1,
                                       data_ids=5,
                                       eco_ids=4,
                                       debug_mode=True)
