...
class DataTablesTransactionsByBlock(DataTablesExt):
...
@app.route("/genesis/database/<int:id>/transactions_by_block/<int:block_id>")
def transactions_by_block(id, block_id):
...
@app.route('/dt/genesis/database/<int:id>/transactions_by_block/<int:block_id>')
def dt_transactions_by_block(id, block_id):
    ...
    query = db.session.query(*columns).with_session(sm.get(id)).filter_by(block_id=block_id)
    params = request.args.to_dict()
    rowTable = DataTablesTransactionsByBlock(params, query, dt_columns)
    rowTable.transactions_post_query_process(hash_ids=3, time_ids=[0],
                                             debug_mode=True)
