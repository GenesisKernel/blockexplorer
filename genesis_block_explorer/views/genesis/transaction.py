...
from ...models.genesis.helpers import TransactionStatusHelper 
...
class DataTablesTransactionStatusHelper(DataTablesExt):
    pass
...
def dt_transaction(id, tx_hash):
    ...
    TransactionStatusHelper.update_from_transactions_status(db_id=id, tx_hash=tx_hash)
    query = db.session.query(*columns).filter_by(db_id=id, tx_hash=tx_hash)
    params = request.args.to_dict()
    rowTable = DataTablesTransactionStatusHelper(params, query, dt_columns)
