...
class DataTablesHashDataTimeMixin:
    def hash_data_time_post_query_process(self, **kwargs):
        logger.debug("hash_data_time_post_query_processkwargs: %s" % kwargs)
        hash_ids = self.prepare_col_ids(col_ids=kwargs.get('hash_ids'))
        if not hash_ids:
            logger.warning("hash_ids isn't set")
        logger.debug("hash_data_time_post_query_process hash_ids: %s" % hash_ids)

        data_ids = self.prepare_col_ids(col_ids=kwargs.get('data_ids'))
        if not data_ids:
            logger.warning("data_ids isn't set")
        logger.debug("hash_data_time_post_query_process data_ids: %s" % data_ids)

        time_ids = self.prepare_col_ids(col_ids=kwargs.get('time_ids'))
        if not time_ids:
            logger.warning("time_ids isn't set")
        logger.debug("hash_time_time_post_query_process time_ids: %s" % time_ids)
        if self.results:
