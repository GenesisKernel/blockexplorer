...
class BlockFiller(Filler):
    def __init__(self, **kwargs):
        kwargs['involved_models'] = kwargs.get('involved_models',
            [BlockModel, HeaderModel, TxModel, ParamModel, ErrorModel])
    ...
    def fill_blocks(self, fetch_from_block_id, fetch_to_block_id, **kwargs):
        ...
        fetch_to_block_id += 1
        logger.debug("fetch_from_block_id: %s fetch_to_block_id: %s fetch_num_of_blocks: %s" % (fetch_from_block_id, fetch_to_block_id, fetch_num_of_blocks))
        for from_block_id in range(fetch_from_block_id, fetch_to_block_id,
            ...
            logger.debug("from_block_id: %s to_block_id: %s cnt: %s" % (from_block_id, to_block_id, cnt))
            try:
                ...
                for block_id in range(from_block_id, to_block_id):
                    ...
                    try:
                        ...
                        BlockModel.create_error_block(block_id, error=str(e),
                                                      raw_error=str(e),
                                          session=self.aux_sm.get(self.seq_num))
        self.unlock(**kwargs)
    ...
    def fill_block(self, block_id, **kwargs):
        ...
        self.fill_blocks(block_id, block_id, **kwargs)
    ...
    def clear(self, **kwargs):
        ...
        session.query(ErrorModel).delete()
        self.unlock(**kwargs)
        self.add_event(caller='clear', stage='finished')

    def cmp_last_block(self, **kwargs):
        pass
